...
export const sendPushToUsers = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: `Push sent to ${count} user(s).`,
    });
  } catch (err) {
...
export const getTripDetails = async (req, res) => {
  try {
    ...
    const driver = trip.assignedDriver
      ? await User.findById(trip.assignedDriver).lean()
      : null;
...
export const getDriverDocuments = async (req, res) => {
  try {
    ...
    if (!documents || documents.length === 0) {
      return res
        .status(404)
        .json({ message: "No documents found for this driver." });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching documents." });
  }
};

/**
 * @desc    Verify or reject a specific driver document
...
export const verifyDriverDocument = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Server error while verifying document." });
  }
};
